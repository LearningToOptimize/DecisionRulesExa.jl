...
function hydro_solution(prob::EmbeddedHydroExaDEProblem, result)
    ...
    if prob.formulation === :dc
        ...
        def_sol    = reshape(sol[off .+ (1:T*nB)],        nB, T);  off += T*nB
        # Canonical ACP keeps reactive nodal balance hard: no reactive slack
        # variables are present in the decision vector.
...
function _build_embedded_ac_hydro_de(
    ...
    # No reactive deficit variable: MAIN-faithful hard reactive nodal balance.
    ...
    # Match PowerModels ACPPowerModel: apparent-power limits at both branch
    # ends, not merely independent bounds on p and q.
