...
"""
    build_hydro_de(power_data, hydro_data, T;
                   ...
                   load_scaler=0.6, strict_targets=true,
                   reactive_deficit_cost=Inf,
...
`deficit_cost` overrides the canonical operational active-deficit coefficient
`power_data.cost_deficit * power_data.baseMVA` (6000 USD/(pu·stage) for MAIN).
...
reactive KCL:
- `nothing` — legacy behavior: one FREE, zero-cost `deficit_q`
...
function build_hydro_de(power_data::PowerData,
                         ...
                         reactive_deficit_cost::Union{Nothing,Real} = Inf,
...
function _build_dc_hydro_de(power_data::PowerData,
    ...
    cd      = float_type(deficit_cost !== nothing ? deficit_cost :
                         power_data.cost_deficit * power_data.baseMVA)
...
function _build_ac_hydro_de(power_data::PowerData,
    ...
    # 7. Apparent-power thermal limits at BOTH branch ends. PowerModels'
    # ACPPowerModel imposes p^2 + q^2 <= rate_a^2 at the from and to ends.
    # Component-wise variable bounds alone describe a square and are strictly
    # weaker; omitting these circles made the Exa AC feasible set larger than
    # the SDDP forward model, especially on deliberately derated corridors.
    thermal_ub = float_type.(repeat([br.rate_a^2 for br in power_data.branches], T))
    thermal_items = [(t = t, br = br_pos)
                     for t in 1:T for br_pos in 1:nBranch]
    ExaModels.constraint(core,
        p_fr[_bri(nBranch, item.t, item.br)]^2 +
        q_fr[_bri(nBranch, item.t, item.br)]^2
        for item in thermal_items;
        lcon = fill(float_type(-Inf), T * nBranch), ucon = thermal_ub,
    )
    ExaModels.constraint(core,
        p_to[_bri(nBranch, item.t, item.br)]^2 +
        q_to[_bri(nBranch, item.t, item.br)]^2
        for item in thermal_items;
        lcon = fill(float_type(-Inf), T * nBranch), ucon = thermal_ub,
    )
    n_con += 2 * T * nBranch

    # 8. Active KCL: pd + gs·vm² − deficit − Σpg + Σp_fr + Σp_to = 0
    ...
    # 9. Reactive KCL: qd − bs·vm² − deficit_q − Σqg + Σq_fr + Σq_to = 0
    ...
    # 10. Initial reservoir condition (skip in strict: reservoir is a parameter,
    # so reservoir[1,r] − p_x0[r] = 0 would be parameter-only — an all-zero
    # Jacobian row. The initial condition is instead maintained by the invariant
    # that prepare_solve! writes p_reservoir[1:nHyd] = x0; see file-top comment.)
    if !strict_targets
    ...
    # 11. Water balance (reservoir is a parameter in strict mode — same expression)
    wb_con_start = n_con + 1
    wb_items = [(res_next  = _ri(nHyd, t+1, r),
                 res_curr  = _ri(nHyd, t,   r),
                 out_idx   = _ri(nHyd, t,   r),
